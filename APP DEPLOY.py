...
# ----------------------------------------------------
# Using relative filename for seamless cloud hosting deployment
...
def initialize_dataset():
    ...
    try:
        ...
        st.error(f"**Permission Denied:** The file at `{DATASET_PATH}` is currently locked by another program. Please close it and refresh!")
        return
    except Exception:
        try:
...
with col_logo2:
    try:
        # Utilizing relative path for image display on server side
...
def get_llm():
    try:
        # Connecting securely to your local Ollama server over your active ngrok public tunnel endpoint
...
# Main medical analysis prompt template
...
# Fixed structuring prompt layout using explicit from_messages mapping to prevent LangChain formatting errors
...
def get_ml_prediction_and_sync(symptoms_text):
    ...
    if llm is not None:
        ...
        try:
            ...
            # Sanitizing string output by cutting away potential code blocks or wrappers
